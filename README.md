...
*   **包棟方案與定價**：依動人數級距（如 10/12/14/16）設定各 tier 價格與房型組合說明。
*   **加人規則**：超過方案基礎人數時，「不多開房」／「多開房」兩種每人加價。
*   **旺季／連假日期區間**：可用後台「新增區間」手動輸入，也可設定 Google 試算表 ID/GID 同步（試算表格式：A欄=類型、B欄=起始日期、C欄=結束日期、D欄=備註）；一般日期（不在旺季/連假）純粹依星期幾判斷平日(日~四)/小假日(五~六)。
*   **測試報價工具**：輸入日期與人數，立即用上面填的資料試算「個別租房」與「包棟」兩種報價，方便驗證資料正確性。

目前報價計算完全由程式碼確定性運算（`src/lib/bookingEngine.ts`），AI 不參與金額計算，避免報價出錯。**這個階段尚未串接 LINE「我要訂房」對話流程與訂單／訂金／退款管理，屬於後續 Phase 2～4 的範圍。**

完整資料表結構請參考 [`supabase_schema.sql`](./supabase_schema.sql) 中的 `room_types`、`room_pricing`、`whole_house_packages`、`whole_house_package_pricing`、`whole_house_extra_person_rules`、`booking_date_ranges`。若您先前已建立過資料庫，請執行該檔案第 7 節的升級用 SQL。
