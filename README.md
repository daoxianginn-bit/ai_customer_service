...
*   **包棟方案與定價（可開關）**：頁面上有「啟用包棟方案」開關，不需要包棟功能的民宿可以直接關掉、整塊設定收起來。包棟本來就是整棟都算在內，只需要設定動人數與各 tier 價格，不用指定房型組合（用哪幾間房是個別租房才需要決定的事，由房型與定價那邊的分配演算法處理）；加人規則就在同一張卡片下方，不用來回切換。
...
*   **測試報價工具**：輸入日期與人數，用畫面上目前（含尚未儲存）的資料試算，方便調整完馬上驗證。
